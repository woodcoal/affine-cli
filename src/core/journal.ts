...
import { createDocFromMarkdownCore, collectDocForMarkdown, ensureNoteBlock, findBlockById, markdownOperationToAppendInput, normalizeAppendBlockInput, createBlock, resolveInsertContext } from '../utils/docsUtil.js';
...
export async function journalListHandler(params: {
	...
	try {
		...
		const { doc: propsDoc, exists: propsDocExists } = await fetchYDoc(socket, workspaceId, docPropsDocId);
...
async function setJournalPropertyInDocProperties(
	...
	const { doc: existingDoc, exists, prevSV } = await fetchYDoc(socket, workspaceId, docPropsDocId);
...
export async function journalAppendHandler(params: {
	...
	try {
		...
		const { doc: doc, exists: snapExists, prevSV: prevSV } = await fetchYDoc(socket, workspaceId, targetDocId);
		if (!snapExists) {
		...
		for (const operation of operations) {
			...
			const input = markdownOperationToAppendInput(operation, targetDocId, workspaceId, false, placement);
			try {
...
export async function journalInfoHandler(params: {
	...
	try {
		...
		const journalDate = await getJournalPropertyFromDocProperties(socket, workspaceId, targetDocId);
...
export async function journalUpdateHandler(params: {
	...
	try {
		...
		const { doc: doc, exists: docExists, prevSV } = await fetchYDoc(socket, workspaceId, targetDocId);
		if (!docExists) {
			throw new Error(`文档 ${targetDocId} 不存在`);
		}
